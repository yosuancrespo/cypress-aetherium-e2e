describe('Play normal level - identify the images to match cards', () => {
  it('passes', () => {
    ...
    cy.get('button.difficulty-button.yellow').contains('Normal').click();
	
    cy.get('button.play-button').contains('Accept').click();
    
	cy.wait(3000);
    
    //Get all 6 card elements and alias them
    cy.get('.MuiGrid-root .MuiBox-root .css-1bkx8aw')
      ...
      .then(($cards) => {
        ...
        //Assert there are exactly 3 matching pairs
        ...
        .then(() => {
          cy.log('All card pairs processed. Expecting Congratulations page for normal level.');
          cy.wait(1500);
          cy.url().should('eq', 'http://localhost:5173/congt-normal');
      });
